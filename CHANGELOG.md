...
- **`backend/local.settings.example.json`**: the README told contributors to copy this file and it did not exist, so the documented quickstart failed at step one. It carries the eight settings the Functions actually read, plus the two the runtime requires (`AzureWebJobsStorage`, `FUNCTIONS_WORKER_RUNTIME`), all with empty or local-emulator placeholders.
- **`E2E_CLERK_USER_EMAIL` in `frontend/.env.local.example`**, which had drifted from `.env.example` and left the Playwright suite's one required variable undocumented in the file contributors are told to copy.
...
- **`@xyflow/react` dropped.** The mind map moved to ECharts and nothing imports xyflow any more; it was still in `dependencies`, pulling `@xyflow/system` and eight more packages into every install.

### Fixed
- **README accuracy.** The quickstart copied a backend settings file that did not exist; the stack listed React Flow instead of ECharts; there was no Node version floor even though Vitest 4 needs 20.12; and the security section claimed transcript, package and chat caches were all identity-keyed when only chat is — the other two are content-addressed, which is deliberate but is a different claim. The testing section now says the E2E job runs in CI, since it does.
...
- **Vacuous E2E tests: 88 `if (await X.isVisible())` guards removed across all 8 spec files.** Each one wrapped an entire test body, so when its selector matched nothing the body never ran and the test still reported green — one spec's only surviving assertion was `expect(true).toBe(true)`. The root cause was that the app has no `data-testid` attributes anywhere, so every `[data-testid="session-card"]`-style selector was dead; the composer specs compounded it by driving `/library` and looking for `input[name="title"]` / `textarea[name="transcript"]`, when the composer is on `/dashboard` and none of its fields carry a `name`. The file input and the "Con IA" switch are `sr-only`, which makes `isVisible()` false by construction and made those bodies unreachable no matter what else was fixed. All eight specs are rewritten against role, label and placeholder locators read off the real components, with backend calls stubbed through `page.route` (CORS headers plus the `OPTIONS` preflight, since the backend is a different origin) and every request body recorded — the composer builds a complete, plausible session from local heuristics when the network is dead, so asserting only on what rendered passes with the endpoint never called. The chromium project now runs 87 real tests. Three tests were dropped as unfixable rather than ported: two asserted a "Reintentar" button and an AI content-filter message that exist nowhere in the composer, and one asserted an "unsupported format" error the app never produces — `validateAudioFile` keys off size alone, and only for files already typed `audio/` or `video/`.
